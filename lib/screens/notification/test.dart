...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('الإشعارات', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
        centerTitle: true,
        actions: [
          IconButton(
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: StreamBuilder<QuerySnapshot>(
            stream: firestore.collection('notifications').orderBy('timestamp', descending: true).snapshots(),
            builder: (context, snapshot) {
